    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.vehicle-documents.update", [$vehicleDocument->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('company') ? 'is-invalid' : '' }}" name="company_id" id="company_id" required>
                    @foreach($companies as $id => $entry)
                        <option value="{{ $id }}" {{ (old('company_id') ? old('company_id') : $vehicleDocument->company->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('company_vehicle') ? 'is-invalid' : '' }}" name="company_vehicle_id" id="company_vehicle_id" required>
                    @foreach($company_vehicles as $id => $entry)
                        <option value="{{ $id }}" {{ (old('company_vehicle_id') ? old('company_vehicle_id') : $vehicleDocument->company_vehicle->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('note') ? 'is-invalid' : '' }}" name="note" id="note">{!! old('note', $vehicleDocument->note) !!}</textarea>
                @if($errors->has('note'))
                    <div class="invalid-feedback">
                        {{ $errors->first('note') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('vehicle_document_type') ? 'is-invalid' : '' }}" name="vehicle_document_type_id" id="vehicle_document_type_id" required>
                    @foreach($vehicle_document_types as $id => $entry)
                        <option value="{{ $id }}" {{ (old('vehicle_document_type_id') ? old('vehicle_document_type_id') : $vehicleDocument->vehicle_document_type->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
